...
async function main() {
  console.log("💰 开始转账代币到 SimpleAirdropV2 合约...\n");
  
  // 读取部署信息
  let deployInfo;
  try {
    deployInfo = JSON.parse(fs.readFileSync('deployment-simple-airdrop-v2.json', 'utf8'));
  } catch (error) {
    console.error("❌ 无法读取部署信息文件 deployment-simple-airdrop-v2.json");
    console.error("   请先运行: npx hardhat run scripts/deploy-simple-airdrop-v2.js --network baseSepolia");
    process.exit(1);
  }
